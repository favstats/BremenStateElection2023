---
title: "2023 Bremen state election"
...
color_dat <- tibble(colors = c("#121212", "#b90e64", "#e00a1e", "#fecb2f", "#7aba2d", "#1a9fdd", "#005ab0",  "#582c83", "#a5defb", "#01acab"),
                    party = c("CDU", "DIE LINKE", "SPD", "FDP", "DIE GRUENEN", "AfD", "BIW", "Volt", "dieBasis", "Tierschutzpartei"))
...
election_dat30 <- readRDS("data/election_dat30.rds") %>% 
  ...
  mutate(party = case_when(
    str_detect(party, "NDNIS 90/DIE")  ~ "DIE GRUENEN",
    str_detect(party, "in Wut")  ~ "BIW",
    T ~ party  )) %>% 
  filter(internal_id != "1790920000965539")%>% 
  filter(internal_id != "21289227249")
...
add_ribbons("CDU", "#121212") %>%
add_ribbons("DIE LINKE", "#b90e64") %>%
add_ribbons("SPD", "#e00a1e") %>%
add_ribbons("FDP", "#fecb2f") %>%
add_ribbons("DIE GRUENEN", "#7aba2d") %>%
# add_ribbons("AfD", "#1a9fdd") %>%
add_ribbons("BIW", "#005ab0") %>%
add_ribbons("Volt", "#582c83") %>%
add_ribbons("dieBasis", "#a5defb") %>%
add_ribbons("Tierschutzpartei", "#01acab")
...
the_order <- contested_dat %>% 
  filter(party == "CDU") %>%   arrange(desc(perc)) %>% 
  pull(value) %>% 
  unique()
